...
function DocumentTypeList() {
  ...
  const sections = [
    ...
    { key: 'Document Type List', content: 'Document Type List', active: true },
  ];
  ...
  const getAllShippingLine = async (fromDate,toDate,search) => {
    try {
      ...
      if (search || (fromDate && toDate)) {
        ...
        response = await apiGET(`/v1/document-type?${queryParams}&limit=${limit}&page=${page}`)
      }
      else {
        response = await apiGET(`/v1/document-type?page=${page}&limit=${limit}`,);
      }
      setLoading(false);
  ...
  const onClickEditButton = (id) => {
    navigate(`/dashboard/document-type/edit/${id}`);
    ...
    <>
      <Sidebar.Pushable>
        <Sidebar
          ...
          onHidden={() => navigate('/dashboard/document-type')}
          vertical={'vertical'}
          visible={visible}>
          <AddDocumentType
            visible={visible}
        ...
        <Sidebar.Pusher dimmed={visible} className="fadeIn">
          <div className="page-header">
            <div>
              ...
              <div className="header-text">All Document Types</div>
              <div className="sub-text">List of all Document Types</div>
            </div>
