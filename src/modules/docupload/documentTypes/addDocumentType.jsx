...
function AddDocumentType(props) {
  ...
  const sections = [
    ...
    { key: id ? 'Edit Document Type' : 'Add Document Type', content: id ? 'Edit Document Type' : 'Add Document Type', active: true },
  ];
  ...
  const onClickGetShippingLineById = async () => {
    try {
      if (loading) return
        const response = await apiGET(`v1/document-type/${id}`)
        setLoading(false)
        if (response.status === 200) {
        setShippingListObj({
  ...
  useEffect(() => {
    ...
    const handleAllExtraction=(id, shippingLineName)=>{
     Swal.fire({
      ...
      if (result.value){
        try {
          const response = await apiPOST(`/v1/document-type/allextraction/${id}`, {
            option: result.value,
          });
          if (response.status === 200) {
            Swal.fire({
    ...
    <div className="fadeIn page-content-wrapper">
      <div className="page-header">
        <div>
          ...
          <div className="header-text">{id ? 'Edit Document Type' : 'Add Document Type'}</div>
