  ...
  const saveBtnClickHandler = async () => {
    ...
    if (response?.status == '200') {
      ...
      if (response?.status == '403') {
        Toast.fire('Error!', response?.data?.message || 'Something went wrong!', 'error');
    ...
    <div className="fadeIn  page-content-wrapper " style={{ paddingBottom: '150px' }}>
      <div className="page-header">
        <div>
          ...
          <div className="sub-text">Upload document to extract the fields</div>
        </div>
      </div>
              ...
              <button
                onClick={() => {
                  if(isFreeAccount && uploadedDocCoount >=10){
                    ...
                    console.log(isFreeAccount, uploadedDocCoount);
