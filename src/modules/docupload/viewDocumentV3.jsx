...
import { ASSIGN_TO, DELETE_DOCUMENT_PAGE, EXTRACTION, REQUEST_TO_DELETE_DOCUMENT_PAGE, REUPLOAD_PDF, SYNC_WITH_ERP, VIEW_RAW, hasAccess } from '../../utils/accessHelper';
import { isElementDuplicated } from '../../utils/utils';
...
export default function ViewDocument() {
    ...
    const pushToOTMClickHandler = async () => {
        ...
        Swal.fire({
            title: `Are you sure? `,
            icon: 'warning',
            text: 'You want to sync this document with ERP ?',
                                ...
                                <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end" }}>
                                    <Dropdown icon='ellipsis vertical'
                                        ...
                                        <Dropdown.Menu>
                                            ...
                                            {
                                                hasAccess(DELETE_DOCUMENT_PAGE)  ? <Dropdown.Item onClick={() => {
                                            ...
                                            {
                                                hasAccess(REQUEST_TO_DELETE_DOCUMENT_PAGE) && <Dropdown.Item onClick={() => {
...
const EditableTextItem = ({ _setDocObj, docObj, compact, field, documentId, thisSelectedSentence, clearSelection, isDuplicate,width }) => {
    ...
    return (
        <div onDoubleClick={handleDoubleClick} style={{width : `${width ? width : null}`}}>
            {isEditing && !docObj?.syncWithOtm ? (
                <div>
                    <textarea
                        onFocus={(e) => {
                            setIsFocused(true)
                ...
                <div style={{ cursor: "pointer", width : `${width ? width : null}`, wordWrap:"break-word"}} ref={divRef}>{isDuplicate && <Label size='small' circular color="red" empty />} {text ? text : "[-NA-]"}</div>
            )}
        </div>
    );
};
...
const EditableMasterItem = ({ _setDocObj, docObj, compact, field, documentId, thisSelectedSentence, clearSelection, domainName, documentNo,width }) => {
    const [isEditing, setIsEditing] = useState(false);
    const [text, setText] = useState(field.overrideValue ? field.overrideValue : field.fieldValue?.toString());
    const [buttonLoading, setButtonLoading] = useState(false);
    const divRef = useRef(null);
    ...
    function getMastersDataBySearch(data) {
        ...
        timeoutRef.current = setTimeout(async () => {
            ...
            if (searchValue) {
                res = await apiGET(
                    `/v1/portmaster/collectionnamelist?collectionName=${collectionName}&search=${searchValue}&domainName=${domainName}&documentNo=${documentNo}`
                );
            }
            else {
                res = await apiGET(
                    `/v1/portmaster/collectionnamelist?collectionName=${collectionName}&domainName=${domainName}&documentNo=${documentNo}`
                );
            }
...
function ShipunitCmp({ _docObj, extractedData = {}, documentId, selectedSentence = '',
    ...
    let renderTable = (aField, idx) => {
        let containerNumbers = aField.fieldValue?.map(obj=>obj.container)
        return <Table compact className='shipUnitTable'>
            <Table.Header>
                <Table.Row>
                    <Table.HeaderCell>Sr. No</Table.HeaderCell>
                    <Table.HeaderCell>Container ({aField.fieldValue?.length})</Table.HeaderCell>
            ...
            <Table.Body>
                {aField.fieldValue?.map((aRow, rowIndex) => {
                    const isDuplicate = isElementDuplicated(containerNumbers, aRow.container)
                    return (
                        <Table.Row key={aRow.itemId}>
                            <Table.Cell>
                                
                                {
                                    !docObj?.syncWithOtm &&  <Button
                            ...
                            <Table.Cell>
                                <EditableTextItem isDuplicate={isDuplicate} compact={true} key={idx + "2"} field={{
                                    fieldValue: aRow.container,
                                    shipUnit: {
                                        itemId: aRow.itemId,
                                        attr: 'container'
                                    }
                                }} docObj={docObj} width="150px" documentId={documentId} thisSelectedSentence={thisSelectedSentence} clearSelection={clearSelection} />
                            </Table.Cell>
                            <Table.Cell>
                                ...
                                }} docObj={docObj} documentId={documentId} thisSelectedSentence={thisSelectedSentence} clearSelection={clearSelection} />

                            </Table.Cell>
                            <Table.Cell
                            // width={1}
                            // style={{ width: '5px', backgroundColor: '#f0f0f0' }}
                            >
    ...
    return <>
        ...
        {_docObj?.shippingLine?.type === 'ocean' && <Scrollbars style={{ display: "flex", flexDirection: "column", justifyContent: "center", marginTop: -10, flex: 1, height: panelHeight, padding: 10 }} autoHide>
            ...
            {
                !docObj?.syncWithOtm &&  <div style={{ marginTop: 16, marginLeft: 10 }}>
                <Button
                    icon size='mini' compact
                    onClick={() => {
                        addNewShipUnit()
