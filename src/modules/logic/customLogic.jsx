...
function CustomLogic() {
            ...
            try {
                if (result.isConfirmed) {
                    ...
                    if (response.status === 200) {
                        ...
                        getAllLogics(fromDate, toDate,search)
                    }
                    else {
                        Swal.fire({
                            title: "Error!",
    ...
    const getAllLogics = async (fromDate, toDate,search) => {
        try {
            setLoading(true);
            let response;
            if (search?.trim() || (fromDate && toDate)) {
                const payload = {
                    search: {
                        name: search.trim(),
                        fromDate: fromDate,
    ...
    useDebounce(() => { getAllLogics(fromDate, toDate,search) }, 300, dependecies);
