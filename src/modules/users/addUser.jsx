...
function AddUser(Props) {
  ...
  const validatePass = (pass1,pass2) => {
    // console.log(pass1,pass2,"okkkkk--->");
    if (!pass1 || pass1.trim() === '') {
  ...
  const onClickAddUser = async () => {
    ...
    function passwordValidationWithRegX(pass) {
          const passwordRegX = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$/;
            setUserObj({ ...userObj, password: pass })

        if (passwordRegX.test(pass)) {
            delete Error?.password
        }else{
            setError({...Error,password :"Required Password 8 Character"})
        }
  ...
  const onClickUpdateUserWithPassword = async () => {
    try {
      const isValid = await validatePass(updatePass,confirmPass)
        // console.log(isValid,updatePass,confirmPass,passError);
        if (isValid) {
            // setLoading(true)
              ...
              />}
          {
            id ? null :
              <Form.Field
                ...
                disabled={id ? true : false}
                onChange={(e) => {
                // passwordValidationWithRegX(e.target.value) for applying validation
...
/>
            ...
            //     required={true}
            //     onFocus={() => { setError(false); }}
            //     value={nodeTreeValue?.length && nodeTreeValue[0]}
            //     onClick={e => {
            //       setOpenModalTree(true)
            //       console.log("e.target", e.target.value);
            //       // setUserObj({ ...userObj, domain: e.target.value })
            //       // setNodeTreeValue(e.target.value)
            //     }}
