...
import { Breadcrumb, Button, Checkbox, Dropdown, Icon, Image, Input, Label, List, Modal, Radio, Sidebar } from 'semantic-ui-react';
...
function UserList(props) {
  ...
  const handleAccessLoginAccessUser = async (userId,checked) => {
    try {
        const response = await apiPOST(`/v1/users/login-access/${userId}`, {inActive:checked})
        if (response.status === 200) {
          getAllUsers()
        }
        else {
  ...
  const columns = [
    ...
    //   ),
        cell: (row) => ( <List style={{ marginTop: 5 }}>
                    {row.domain.map((location)=><List.Item>
    ...
    {
      name: 'Last Login On',
      selector: (row) => row?.lastLogin ? <><p>{moment(row?.lastLogin).format('DD/MM/YYYY')}</p>{moment(row?.lastLogin).format('HH:mm a')}</>:"--",
    },
    {
      name: <p>Log in Acess</p>,
      cell: (row) =><div> <Checkbox toggle checked={!(row?.inActive)} onChange={(e,{checked})=>handleAccessLoginAccessUser(row._id,!checked)} /> <p>{row?.inActive ? "Deactive":"Active"} </p>  </div>,
    },
    {
      name: 'Action',
      cell: (row) => (
