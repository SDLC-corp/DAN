...
function AddShippingLine(props) {
  ...
  const sections = [
    ...
    { key: 'Document Type List', content: 'Document Type List', link: true },
    { key: id ? 'Edit Document Types' : 'Add Document Types', content: id ? 'Edit Document Types' : 'Add Document Types', active: true },
  ];
    ...
    try {
      ...
      if (isValid) {
        ...
        if (response.status === 200) {
          Swal.fire({
            title: "Success!",
            text: "Shipping Line added successfully",
            icon: "success",
          });
          setLoading(false)
          clearFields()
    ...
    //   if (loading) return
      ...
      if (isValid) {
        ...
        if (response.status === 200) {
          Swal.fire({
            title: "Success!",
            text: "Shipping Line updated successfully",
            icon: "success",
          });
          props.getAllShippingLine()
          setLoading(false)
      ...
      if (result.value){
        try {
          ...
          if (response.status === 200) {
            Swal.fire({
              title: 'Success!',
              text: 'All Shipping Line Extraction successfully',
              icon: 'success',
            });
          } else {
            Swal.fire({
    ...
    <div className="fadeIn page-content-wrapper">
      <div className="page-header">
        <div>
          ...
          <div className="header-text">{id ? 'Edit Document Types' : 'Add Document Types'}</div>
          <div className="sub-text">Proceed to {id ? 'edit Document Type here' : 'add new Document Type here'}</div>
        </div>
      ...
      <div className="page-footer ">
       <div>
        ...
        onClick={id ? onClickEditShippingLineById : onClickAddShippingLine} disabled={loading} loading={loading}>
          {id ? 'Update shipping line' : 'Create shipping line'}
        </button>
