...
const AddAirPortMaster = (props) => {
    ...
    const clearFields = () => {
        ...
        setErrorObj()
        navigate('/dashboard/air-portmaster')
    }
    ...
    const onClickAddAirPortMaster = async () => {
        ...
        try {
            ...
            if (isValid) {
                setLoading(true)
                const response = await apiPOST('/v1/air-portmaster', dataObj)
                setLoading(false)
    ...
    const onClickEditPortMasterById = async () => {
        try {
            ...
            if (isValid) {
                setLoading(true)
                const response = await apiPOST(`/v1/air-portmaster/${id}`, dataObj)
                setLoading(false)
    ...
    const onClickGetPortMasterById = async () => {
        try {
            if (loading) return
            const response = await apiGET(`v1/air-portmaster/${id}`)
            setLoading(false)
            if (response.status === 200) {
                setDataObj({
        ...
        <div className="fadeIn page-content-wrapper ">
            ...
            <div className="page-footer ">
                ...
                <button
                    ...
                    onClick={id ? onClickEditPortMasterById : onClickAddAirPortMaster}
                    disabled={loading} loading={loading}
                >
...
export default AddAirPortMaster