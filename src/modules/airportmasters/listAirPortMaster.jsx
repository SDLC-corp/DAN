...
import AddAirPortMaster from './addAirPortMaster';
import { AuthContext } from '../../contexts';
import { ADD_AIR_PORT_MASTERS, DELETE_AIR_PORT_MASTERS, EDIT_AIR_PORT_MASTERS, hasAccess } from '../../utils/accessHelper';

function ListAirPortMaster() {
    ...
    const sections = [
        ...
        { key: 'Air Port Master List', content: 'Air Port Master List', active: true },
    ];


    const onClickEditButton = (id) => {
        navigate(`/dashboard/air-portmaster/edit/${id}`);
    }
    useEffect(() => {
        ...
        Swal.fire({
            ...
            try {
                if (result.isConfirmed) {
                    const response = await apiDELETE(`v1/air-portmaster/${id}`)
                    if (response.status === 200) {
                        ...
                        getAirPortMasterList(fromDate, toDate,search)
                    }
                    else {
                        Swal.fire({
                            title: "Error!",
    ...
    const getAirPortMasterList = async (fromDate, toDate,search) => {
        try {
            setLoading(true);
            let response;
            if (search || (fromDate && toDate)) {
                ...
                response = await apiGET(`/v1/air-portmaster/list?${queryParams}&limit=${limit}&page=${page}`)
            }
            else {
                response = await apiGET(`/v1/air-portmaster/list/?page=${page}&limit=${limit}`);
            }
            setLoading(false);
            if (response.status === 200) {
    ...
    const getSyncPortMasterList = async () => {
        try {
            setSyncLoading(true);
            let response = await apiGET(`/v1/air-portmaster/syncport`);
            setSyncLoading(false);

            if (response.status === 200) {
                Swal.fire({
                    title: "Done!",
                    text: "Synchronization Successfully Done!",
                    icon: "success",
                });
                getAirPortMasterList(fromDate, toDate,search)
            }
    ...
    const columns = [
        ...
        },
        {
            ...
            { 
                hasAccess(EDIT_AIR_PORT_MASTERS) && <button className="ui blue icon button basic" onClick={ () => {onClickEditButton(row._id)}}>
                <i className="edit icon"></i>
            </button>
            }
            {
                hasAccess(DELETE_AIR_PORT_MASTERS) && <button className="ui red icon button basic" onClick={() => {onClickDeleteButton(row._id)} }>
    ...
    useDebounce(() => { getAirPortMasterList(fromDate, toDate,search) }, 300, dependecies);
        ...
        <>
            ...
            <Sidebar.Pushable>
                <Sidebar
                    ...
                    onHidden={() => navigate('/dashboard/air-portmaster')}
                    vertical={'vertical'}
                    visible={visible}>
                    <AddAirPortMaster
                        visible={visible}
                        getAirPortMasterList={getAirPortMasterList}
                    />
                </Sidebar>
                <Sidebar.Pusher dimmed={visible} className="fadeIn">
                    <div className="page-header">
                        <div>
                            ...
                            <div className="header-text">All Air Port Master list</div>
                            <div className="sub-text">List of all Air Port Master List</div>
                        </div>
                        ...
                        {
                            hasAccess(ADD_AIR_PORT_MASTERS) && <Button
                                primary
                                onClick={() => {
                                    navigate('/dashboard/air-portmaster/add');
                                }}>
                                Add Port
                            </Button>
                        }   
...
export default ListAirPortMaster;
