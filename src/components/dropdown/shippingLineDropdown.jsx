...
const ShippingLineNameDropdown = ({
    shippingLineId,shippingId,refresh,multiselect,setSelectedShippingLineName,
    type
}) => {
    ...
    const getAllShippingLineList = async() => {
        try {
          let response
            if (type == "air" || type == "ocean") {
              response = await apiGET(`/v1/shipping-lines?type=${type}`);
            }else{
              response = await apiGET(`/v1/shipping-lines/list`);
            }
    ...
    return (
        // <div className="ui toggle checkbox small">
            <Dropdown
              ...
              placeholder='Select Carrier Line'
