...
const InvoiceUpload = () => {
  ...
  async function verifyDocument(orderBaseXid, domainName) {
    ...
    let response = await apiGET(`/v1/documents/check/orderBases?documentNo=${orderBaseXid}&domainName=${domainName}&type=${extractionType}`);
  ...
  const saveBtnClickHandler = async () => {
    ...
    setLoading(true);
    let response = await apiPOST(`/v1/documents?extractionType=${"invoice"}`, payload);
    setLoading(false);
        ...
        <div className="eight wide column">
          <Form style={{ width: '100%' }}>
            ...
            {/* <Form.Dropdown
                ...
                //     setErrorObj();
                //   }}
                  value={docObj.document}
                  onChange={(e, data) => {
                    updateDocObj("document", data.value);
                    console.log("on select", data.value);
                  }}
                /> */}
            ...
            {/* <Form.Dropdown
                  ...
                  options={extractionType == "ocean"?oceanDocumentType:airDocumentType}
                  required={true}
                  error={errorObj && errorObj.fieldId}
                  selection
                  onFocus={() => {
            ...
            {/* <Form.Dropdown
                  ...
                  options={[{ text: "Draft", value: "draft" }, { text: "Original", value: "original" }]}
                  required={true}
                  error={errorObj && errorObj.fieldId}
                  selection
                  onFocus={() => {
            ...
            {/* {(user.role != "documentation") &&<>
                    ...
                    <UserListDropDown setAssignToUserId={setAssignToUserId} assignToUserId={assignToUserId} disabled={loading}/>
                </>} */}
